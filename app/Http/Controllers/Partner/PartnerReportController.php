...
class PartnerReportController extends Controller
{
    ...
    public function index()
    {
        ...
        abort_unless($partner->canDownloadReports(), 403, 'Report download not permitted for this partner.');
        ...
        return view('partner.reports.index', compact(
            'sessions', 'activeSession', 'courseTypes', 'courses', 'streams', 'studentStatuses'
        ));
    }
    ...
    public function downloadStudents(Request $request)
    {
        ...
        $filterSummary = $this->resolveStudentFilterLabels($request, $partner->institute_id);

        if ($format === 'pdf') {
            $institute = Institute::find($partner->institute_id);
            $pdf = Pdf::loadView('partner.reports.pdf.students', compact(
                'students', 'partner', 'institute', 'filterSummary'
    ...
    public function downloadAdmissions(Request $request)
    {
        ...
        if ($format === 'pdf') {
            $institute = Institute::find($partner->institute_id);
            $pdf = Pdf::loadView('partner.reports.pdf.admissions', compact(
                'students', 'partner', 'institute', 'filterSummary'
            ))->setPaper('A4', 'landscape');
    ...
    public function downloadFeeCollection(Request $request)
    {
        ...
        $query = FeeInvoice::where('institute_id', $partner->institute_id)
        ...
        $invoices = $query->get();
        $filterSummary = $this->resolveFeeFilterLabels($request, $partner->institute_id);

        if ($format === 'pdf') {
            $institute = Institute::find($partner->institute_id);
            $pdf = Pdf::loadView('partner.reports.pdf.fee-collection', compact(
                'invoices', 'partner', 'institute', 'filterSummary'
            ))->setPaper('A4', 'landscape');
