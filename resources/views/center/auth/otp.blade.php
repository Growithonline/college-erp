...
<head>
    ...
    <title>Center OTP Verification</title>
    ...
    <style>
        ...
        .otp-chip{display:inline-flex;align-items:center;gap:8px;padding:6px 12px;border-radius:999px;background:#185FA515;color:#185FA5;font-size:12px;font-weight:600;}
        ...
        .btn-submit{height:46px;border-radius:8px;font-size:15px;font-weight:500;background:#185FA5;border:none;transition:background .18s ease,box-shadow .18s ease,transform .15s ease;}
        .btn-submit:hover{background:#1450893;box-shadow:0 4px 14px rgba(24,95,165,.35);transform:translateY(-1px);}
    ...
    <div class="p-4 p-md-5">
        <div class="text-center mb-4">
            ...
            <p class="text-muted mb-0 small">
                A 6-digit OTP has been sent to <strong>{{ $center->email }}</strong>.
                @if($center->mobile)
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <a href="{{ route('center.login') }}" class="small text-muted text-decoration-none">
                <i class="bi bi-arrow-left me-1"></i>Back to login
            </a>
            <form method="POST" action="{{ route('center.otp.resend') }}">
                @csrf
