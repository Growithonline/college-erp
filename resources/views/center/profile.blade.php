...
<style>
.profile-avatar { width:80px;height:80px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:700;color:#fff;background:linear-gradient(135deg,#0d3b75,#185FA5);margin:0 auto 12px;flex-shrink:0; }
.info-row { display:flex;align-items:flex-start;gap:12px;padding:10px 0;border-bottom:1px solid #f1f5f9; }
.info-row:last-child { border-bottom:none; }
.info-icon { width:32px;height:32px;border-radius:8px;background:#eff6ff;color:#185FA5;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0; }
    ...
    <div class="col-12 col-lg-4">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body p-4 text-center">
                <div class="profile-avatar">{{ strtoupper(substr($center->name, 0, 1)) }}</div>
                <h5 class="fw-bold mb-1" style="color:#1e293b;">{{ $center->name }}</h5>
                <span class="badge px-3 py-1 rounded-pill mb-3"
                      style="background:#eff6ff;color:#185FA5;font-size:12px;">
                    <i class="bi bi-building me-1"></i>Center
                </span>
                ...
                <div class="text-start mt-3">
                    <div class="info-row">
                        ...
                        <div>
                            ...
                            <div class="info-val">{{ $center->email ?: '—' }}</div>
                        </div>
                    </div>
                    <div class="info-row">
                        ...
                        <div>
                            ...
                            <div class="info-val">{{ $center->mobile ?: '—' }}</div>
                        </div>
                    </div>
                    <div class="info-row">
                        ...
                        <div>
                            ...
                            <div class="info-val">{{ $center->institute?->name ?? '—' }}</div>
                        </div>
                    </div>
                    @if($center->city || $center->state)
                    <div class="info-row">
                        <div class="info-icon"><i class="bi bi-geo-alt-fill"></i></div>
                        <div>
                            <div class="info-label">Location</div>
                            <div class="info-val">{{ collect([$center->city, $center->state])->filter()->implode(', ') }}</div>
                        </div>
                    </div>
                    @endif
                    @if($center->address)
                    <div class="info-row">
                        ...
                        <div>
                            ...
                            <div class="info-val">{{ $center->address }}</div>
                        </div>
                    </div>
                    @endif
                    <div class="info-row">
    ...
    <div class="col-12 col-lg-8">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-bottom px-4 py-3">
                <div class="d-flex align-items-center gap-2">
                    <div style="width:34px;height:34px;border-radius:8px;background:#eff6ff;display:flex;align-items:center;justify-content:center;">
                        <i class="bi bi-lock-fill" style="color:#185FA5;font-size:15px;"></i>
                    </div>
                    <div>
            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('center.change-password.update') }}" id="pwdForm">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                    ...
                    <button type="submit" class="btn w-100 text-white fw-semibold" style="background:#185FA5;border-radius:10px;padding:10px;">
        ...
        <div class="card border-0 shadow-sm mt-4">
            <div class="card-body p-4">
                <div class="fw-bold mb-2" style="font-size:13px;color:#1e293b;"><i class="bi bi-info-circle-fill text-primary me-1"></i>Password Tips</div>
