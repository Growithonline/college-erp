...
<style>
    ...
    .logo-box { width:46px; height:46px; border:1px solid #a7f3d0; border-radius:8px;
                text-align:center; line-height:46px; font-size:15px; font-weight:800; color:#059669; }
    ...
    .filter-bar { background:#f0fdf4; border:1px solid #a7f3d0; border-radius:5px;
    ...
    table.data thead th {
        background:#047857; color:#fff; font-size:7.5px; font-weight:700;
    ...
    .uid { background:#ecfdf5; color:#065f46; border:1px solid #a7f3d0;
...
@php
    ...
    $byCourse = $students->groupBy(fn($s) => $s->stream->course->name ?? 'Unknown');
@endphp
<div style="margin-bottom:8px;">
    ...
    <span class="s-chip">
        ...
        <span class="val" style="color:#047857;">{{ $group->count() }}</span>
    </span>
    @endforeach
    @foreach($byCourse->take(5) as $cname => $grp)
    <span class="s-chip">
        <span class="lbl" style="font-size:6.5px;">{{ $cname }}</span><br>
        <span class="val" style="color:#1d4ed8; font-size:11px;">{{ $grp->count() }}</span>
    </span>
    @endforeach
</div>
...
<table class="footer-line" cellpadding="0" cellspacing="0">
    <tr>
        <td>{{ $institute->name }} &mdash; {{ $center->name }} &nbsp;|&nbsp; Admission Report</td>
