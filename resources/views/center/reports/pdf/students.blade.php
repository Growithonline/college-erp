...
<style>
    ...
    .header { border-bottom: 2.5px solid #2563eb; padding-bottom: 7px; margin-bottom: 8px; }
    .header-table { width:100%; }
    .header-table td { vertical-align:middle; }
    .logo-box { width:46px; height:46px; border:1px solid #bfdbfe; border-radius:8px;
                text-align:center; line-height:46px; font-size:15px; font-weight:800; color:#2563eb; }
    ...
    .report-sub   { font-size:9px; color:#2563eb; font-weight:600; margin-top:2px; }
    .meta-right   { text-align:right; font-size:8px; color:#475569; line-height:1.9; }
    .meta-right strong { color:#0f172a; }

    .filter-bar { background:#eff6ff; border:1px solid #bfdbfe; border-radius:5px;
                  padding:5px 8px; margin-bottom:8px; }
    .filter-bar-inner { display:inline; }
    .f-chip { display:inline; font-size:7.5px; color:#1d4ed8; margin-right:12px; }
    .f-chip strong { color:#1e3a8a; }
    ...
    .s-chip .val { font-size:12px; font-weight:800; color:#1e40af; }

    table.data { width:100%; border-collapse:collapse; }
    table.data thead th {
        background:#1d4ed8; color:#fff; font-size:7.5px; font-weight:700;
    ...
    .uid { background:#eff6ff; color:#1d4ed8; border:1px solid #bfdbfe;
...
<div class="filter-bar" style="margin-bottom:8px;">
    <span style="font-size:7.5px; color:#1e40af; font-weight:700; margin-right:8px;">FILTERS:</span>
...
{{-- ── Summary Chips ── --}}
@php
    $byStatus = $students->groupBy('status');
    $statColors = ['active'=>'#dcfce7','inactive'=>'#f3f4f6','detained'=>'#fee2e2','passed_out'=>'#e0f2fe','transferred'=>'#fef3c7','cancelled'=>'#fce7f3'];
@endphp
<div style="margin-bottom:8px;">
    ...
    <span class="s-chip">
        <span class="lbl">{{ ucfirst(str_replace('_',' ',$status)) }}</span><br>
        <span class="val" style="font-size:12px; font-weight:800; color:#1e40af;">{{ $group->count() }}</span>
    </span>
    @endforeach
    @php $genderGroups = $students->groupBy('gender'); @endphp
    @foreach($genderGroups as $g => $grp)
    <span class="s-chip">
        <span class="lbl">{{ ucfirst($g ?: 'Unknown') }}</span><br>
        <span class="val" style="font-size:12px; font-weight:800; color:#0369a1;">{{ $grp->count() }}</span>
    </span>
    @endforeach
</div>
...
<table class="data" cellspacing="0" cellpadding="0">
    ...
    <tbody>
        ...
        @php
            $st   = $s->status ?? 'active';
            $bCls = 'b-' . (in_array($st, ['active','inactive','detained','passed_out','transferred','cancelled']) ? $st : 'default');
        @endphp
        <tr>
            <td class="num">{{ $i + 1 }}</td>
            <td><span class="uid">{{ $s->student_uid ?? '—' }}</span></td>
            <td>
                <div class="fw">{{ $s->name }}</div>
...
<table class="footer-line" cellpadding="0" cellspacing="0">
    <tr>
        <td>{{ $institute->name }} &mdash; {{ $center->name }} &nbsp;|&nbsp; Student List Report</td>
