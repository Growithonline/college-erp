...
<head>
<meta charset="UTF-8">
<title>Channel Partner Collection Report</title>
<style>
...
<div class="hdr">
    ...
    <div class="hdr-m">
        ...
        <div class="inst-sub">Channel Partner Collection Report</div>
    </div>
    <div class="hdr-r">
...
<div class="summary">
    ...
    <div class="summary-box"><div class="lbl">Partner Count</div><div class="val">{{ number_format($partnerData->count()) }}</div></div>
</div>

<div class="section-title">Partner-wise Collection</div>
<table>
    <thead>
        <tr>
            <th style="width:4%">#</th>
            <th style="width:22%">Channel Partner</th>
    ...
    <tbody>
        ...
        <tr>
            <td class="c">{{ $i + 1 }}</td>
            <td><strong>{{ $row['partner']?->name ?? 'Unknown Partner' }}</strong></td>
        ...
        @empty
        <tr><td colspan="11" class="c">No channel partner collections found for this date range.</td></tr>
        @endforelse
    </tbody>
    ...
    <tfoot>
        <tr>
            ...
            <td class="r">{{ number_format($partnerData->sum('cash'), 2) }}</td>
            <td class="r">{{ number_format($partnerData->sum('upi'), 2) }}</td>
            <td class="r">{{ number_format($partnerData->sum('online'), 2) }}</td>
            <td class="r">{{ number_format($partnerData->sum('cheque'), 2) }}</td>
            <td class="r">{{ number_format($partnerData->sum('dd'), 2) }}</td>
            <td class="r">{{ number_format($partnerData->sum('neft'), 2) }}</td>
            <td class="r">{{ number_format($partnerData->sum('rtgs'), 2) }}</td>
...
{{-- Bank-wise breakdown fixed at the bottom, after the partner list --}}
<div class="bank-section">
    <div class="section-title">Bank-wise Collection — Channel Partner Breakdown</div>
    <table>
        <thead>
            <tr>
                <th style="width:40%">Bank / Account</th>
                <th style="width:30%">Channel Partner</th>
        ...
        <tbody>
            ...
            <tr>
                <td></td>
                <td>{{ $pr['name'] }}</td>
                <td class="c">{{ $pr['count'] }}</td>
                <td class="r">{{ number_format($pr['total'], 2) }}</td>
            </tr>
            @endforeach
            @empty
