...
<head>
    <meta charset="UTF-8">
    <title>{{ $pageTitle ?? 'Admission Report' }}</title>
    <style>
...
@php
    ...
    $type        = $type        ?? '';
    $pageTitle   = $pageTitle   ?? 'Admission Report';
@endphp
...
<div class="hdr">
    ...
    <div class="hdr-m">
        ...
        <div class="inst-sub">{{ $pageTitle }} &mdash; {{ $sessionObj?->name ?? 'All Sessions' }}</div>
    </div>
    <div class="hdr-r">
...
<table class="t" cellspacing="0" cellpadding="0">
    <colgroup>
        ...
        @if($type === 'blocked') <col style="width:46px;"> @endif {{-- Reason --}}
    </colgroup>
    <thead>
        <tr>
            <th class="c">#</th>
            ...
            <th>Status</th>
            @if($type === 'blocked') <th>Reason</th> @endif
        </tr>
    </thead>
    <tbody>
            ...
            $pdfAdmittedBy = $student->admittedBy?->name
                ?? match($pdfSrc) {
                    'center'                     => ($centersMap[$student->admission_source_id]  ?? null)
                                                    ? \Illuminate\Support\Str::limit($centersMap[$student->admission_source_id], 20)
                                                    : 'Center',
                    'partner', 'channel_partner' => ($partnersMap[$student->admission_source_id] ?? null)
                                                    ? \Illuminate\Support\Str::limit($partnersMap[$student->admission_source_id], 20)
                                                    : 'Partner',
        ...
        <tr>
            ...
            <td>
                {{ $student->coursePart?->year_label ?? '—' }}@if($student->current_semester)<span class="sub">S{{ $student->current_semester }}</span>@endif
            </td>
            <td>{{ ucfirst(substr($student->gender ?? '—', 0, 1)) }}</td>
            <td class="c">{{ strtoupper($student->category ?? '—') }}</td>
            ...
            @if($type === 'blocked')
                <td class="wrap">{{ \Illuminate\Support\Str::limit($student->status_reason ?? '—', 30) }}</td>
            @endif
        </tr>
        @empty
        <tr>
...
{{-- ── FOOTER ──────────────────────────────────────────────── --}}
<div class="ftr">
    <div class="ftr-l">{{ $institute->name }} &mdash; {{ $pageTitle }} &mdash; Confidential</div>
