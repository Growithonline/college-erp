...
<head>
    ...
    <title>OTP Verification — Channel Partner</title>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;}
        ...
        .bg-glow1{position:fixed;z-index:2;pointer-events:none;width:500px;height:500px;border-radius:50%;background:radial-gradient(circle,rgba(92,49,9,.6) 0%,transparent 70%);top:-150px;left:-100px;animation:floatOrb 8s ease-in-out infinite;}
        .bg-glow2{position:fixed;z-index:2;pointer-events:none;width:400px;height:400px;border-radius:50%;background:radial-gradient(circle,rgba(133,79,11,.45) 0%,transparent 70%);bottom:-120px;right:-80px;animation:floatOrb 10s ease-in-out infinite reverse;}
        ...
        .otp-input:focus{border-color:#854F0B;box-shadow:0 0 0 3px rgba(133,79,11,.12);background:#fff;outline:none;}
        .otp-input.is-invalid{border-color:#ef4444;}
        .btn-submit{height:46px;border-radius:10px;font-size:14px;font-weight:600;background:linear-gradient(135deg,#5c3109,#854F0B);border:none;color:#fff;letter-spacing:.02em;transition:all .18s ease;width:100%;}
        .btn-submit:hover{background:linear-gradient(135deg,#3d1f05,#6b3f09);box-shadow:0 6px 20px rgba(133,79,11,.35);transform:translateY(-1px);}
        ...
        .back-link:hover{color:#854F0B;}
        .resend-btn{font-size:12px;color:#854F0B;background:none;border:none;padding:0;cursor:pointer;font-weight:600;}
        .resend-btn:hover{color:#5c3109;}
...
<div class="login-wrap">
    <div class="login-card">
        ...
        <div class="card-head">
            ...
            <div class="email-chip"><i class="bi bi-envelope-fill"></i>{{ $partner->email }}</div>
        </div>
        ...
        <div class="card-body-wrap">
            ...
            <form method="POST" action="{{ route('partner.otp.verify') }}">
                @csrf
                <div class="mb-4">
