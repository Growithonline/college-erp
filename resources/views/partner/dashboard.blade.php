...
<style>
.hero-card { background:linear-gradient(135deg,#5c3109 0%,#854F0B 55%,#b06e14 100%); border-radius:16px; padding:26px 28px; color:#fff; margin-bottom:24px; position:relative; overflow:hidden; }
...
<div class="hero-card mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            ...
            <h4 class="mb-0 fw-bold" style="font-size:1.3rem;">{{ $partner->name }}</h4>
            <div style="font-size:12px;opacity:.7;margin-top:2px;">
                ...
                @if($partner->commission_percent > 0)
                    &nbsp;·&nbsp; <i class="bi bi-percent me-1"></i>{{ $partner->commission_percent }}% Commission
                @endif
                &nbsp;·&nbsp;
        ...
        <div class="card kpi-card h-100">
            <div class="card-body">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <div class="text-muted" style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.4px;">My Commission</div>
                        <div class="fw-bold" style="font-size:1.4rem;line-height:1.2;color:#7c3aed;">₹{{ number_format($totalCommission) }}</div>
                        <div class="text-muted" style="font-size:10px;">{{ $partner->commission_percent }}% of collections</div>
                    </div>
                </div>
            </div>
        </div>
...
<div class="alert d-flex align-items-center gap-3 border-0 rounded-3 shadow-sm mb-4"
    ...
    <a href="{{ route('partner.fee.wallet.status') }}" class="btn btn-sm btn-outline-warning flex-shrink-0" style="font-size:12px;">
...
@php
    $actions = [];
    if($canAdmit)
        $actions[] = ['route'=>route('partner.admissions.quick-create'),'label'=>'Quick Register','sub'=>'Add student fast','icon'=>'bi-lightning-fill','color'=>'#f59e0b','bg'=>'#fffbeb'];
    if($canAdmit && $partner->canUseFullAdmissionForm())
        $actions[] = ['route'=>route('partner.admissions.create'),'label'=>'Full Admission','sub'=>'Complete form','icon'=>'bi-person-plus-fill','color'=>'#3b82f6','bg'=>'#eff6ff'];
    if($canViewStudents) {
        $actions[] = ['route'=>route('partner.students.index'),'label'=>'My Students','sub'=>'View all students','icon'=>'bi-people-fill','color'=>'#06b6d4','bg'=>'#ecfeff'];
        $actions[] = ['route'=>route('partner.students.search'),'label'=>'Global Search','sub'=>'Find any student','icon'=>'bi-search','color'=>'#6366f1','bg'=>'#eef2ff'];
    }
    if($canCollectFee && (!$w || !$wBlocked || !$wBlocked['blocked']))
        $actions[] = ['route'=>route('partner.fee.create'),'label'=>'Collect Fee','sub'=>'Accept payment','icon'=>'bi-cash-coin','color'=>'#10b981','bg'=>'#f0fdf4'];
    if($canCollectFee)
        $actions[] = ['route'=>route('partner.fee.index'),'label'=>'My Collections','sub'=>'Fee history','icon'=>'bi-receipt-cutoff','color'=>'#475569','bg'=>'#f1f5f9'];
    if($w)
        $actions[] = ['route'=>route('partner.fee.wallet.status'),'label'=>'Fee Wallet','sub'=>'Wallet status','icon'=>'bi-wallet2','color'=>'#8b5cf6','bg'=>'#f5f3ff'];
@endphp

@if(count($actions))
<div class="sec-head">
    <span class="sec-head-title"><i class="bi bi-grid-1x2-fill" style="color:#854F0B;"></i> Quick Actions</span>
    <hr>
</div>
<div class="row g-3 mb-4">
...
<div class="card border-0 shadow-sm mt-4">
    <div class="card-header bg-white border-bottom py-3 px-4">
        <div class="d-flex align-items-center gap-2">
            <i class="bi bi-person-check" style="color:#854F0B;"></i>
                        ...
                        <td class="ps-4">
                            <div class="d-flex align-items-center gap-2">
                                <div class="rounded-circle d-flex align-items-center justify-content-center text-white fw-bold flex-shrink-0"
                                     style="width:28px;height:28px;font-size:10px;background:#854F0B;">
