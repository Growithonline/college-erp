...
<style>
    ...
    .header { border-bottom: 2.5px solid #059669; padding-bottom: 7px; margin-bottom: 8px; }
    .header-table { width:100%; }
    .header-table td { vertical-align:middle; }
    .logo-box { width:46px; height:46px; border:1px solid #a7f3d0; border-radius:8px; text-align:center; line-height:46px; font-size:15px; font-weight:800; color:#059669; }
    ...
    .report-sub   { font-size:9px; color:#059669; font-weight:600; margin-top:2px; }
    .meta-right   { text-align:right; font-size:8px; color:#475569; line-height:1.9; }
    .meta-right strong { color:#0f172a; }
    .filter-bar { background:#f0fdf4; border:1px solid #a7f3d0; border-radius:5px; padding:5px 8px; margin-bottom:8px; }
    .f-chip { display:inline; font-size:7.5px; color:#065f46; margin-right:12px; }
    .f-chip strong { color:#064e3b; }
    ...
    table.data thead th { background:#047857; color:#fff; font-size:7.5px; font-weight:700; padding:4.5px 4px; text-align:left; white-space:nowrap; }
    ...
    .uid { background:#ecfdf5; color:#065f46; border:1px solid #a7f3d0; border-radius:3px; padding:1px 4px; font-size:7.5px; font-weight:700; }
...
<div class="header">
    <table class="header-table" cellpadding="0" cellspacing="0">
        <tr>
            ...
            <td>
                <div class="report-title">Admission Report</div>
                <div class="report-sub">{{ $institute->name }} &nbsp;|&nbsp; Partner: {{ $partner->name }}</div>
            </td>
            <td class="meta-right">
                <div>Total Admissions: <strong>{{ $students->count() }}</strong></div>
...
<div class="filter-bar">
    <span style="font-size:7.5px; color:#064e3b; font-weight:700; margin-right:8px;">FILTERS:</span>
...
@php $byStatus = $students->groupBy('status'); $byCourse = $students->groupBy(fn($s) => $s->stream->course->name ?? 'Unknown'); @endphp
<div style="margin-bottom:8px;">
    ...
    <span class="s-chip"><span class="lbl">{{ ucfirst(str_replace('_',' ',$status)) }}</span><br><span class="val" style="color:#047857;">{{ $group->count() }}</span></span>
    @endforeach
    @foreach($byCourse->take(5) as $cname => $grp)
    <span class="s-chip"><span class="lbl" style="font-size:6.5px;">{{ $cname }}</span><br><span class="val" style="color:#1d4ed8; font-size:11px;">{{ $grp->count() }}</span></span>
    @endforeach
</div>
...
<table class="footer-line" cellpadding="0" cellspacing="0">
    <tr>
        <td>{{ $institute->name }} &mdash; Partner: {{ $partner->name }} &nbsp;|&nbsp; Admission Report</td>
