...
<style>
    ...
    .logo-box { width:46px; height:46px; border:1px solid #fde68a; border-radius:8px; text-align:center; line-height:46px; font-size:15px; font-weight:800; color:#d97706; }
    ...
    .filter-bar { background:#fffbeb; border:1px solid #fde68a; border-radius:5px; padding:5px 8px; margin-bottom:8px; }
    ...
    table.data thead th { background:#b45309; color:#fff; font-size:7.5px; font-weight:700; padding:4.5px 4px; text-align:left; white-space:nowrap; }
    ...
    .inv { background:#fffbeb; color:#92400e; border:1px solid #fde68a; border-radius:3px; padding:1px 4px; font-size:7.5px; font-weight:700; }
    ...
    tfoot td  { background:#fef3c7; font-weight:700; font-size:8.5px; padding:5px 4px; border-top:2px solid #d97706; }
...
@php $byMode = $invoices->groupBy('payment_mode'); @endphp
<div style="margin-bottom:8px;">
    ...
    <span class="s-chip">
        ...
        <span class="val" style="color:#b45309; font-size:10px;">{{ $group->count() }} &mdash; Rs {{ number_format($group->sum('paid_amount'), 0) }}</span>
    </span>
    @endforeach
</div>
...
<table class="data" cellspacing="0" cellpadding="0">
    ...
    <tbody>
        ...
        @php $mode = $inv->payment_mode ?? 'cash'; $st = $inv->student; @endphp
        <tr>
            <td class="num">{{ $i + 1 }}</td>
            ...
            <td><span class="mode-badge m-{{ $mode }}">{{ strtoupper($mode) }}</span></td>
...
<table class="footer-line" cellpadding="0" cellspacing="0">
    <tr>
        <td>{{ $institute->name }} &mdash; Partner: {{ $partner->name }} &nbsp;|&nbsp; Fee Collection Report</td>
