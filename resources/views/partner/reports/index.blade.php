...
.report-icon { width:40px; height:40px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:18px; }
...
.report-footer { padding:12px 18px; background:#fff; border-radius:0 0 10px 10px; }
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <small class="text-muted">Filter deeply and download CSV, Excel or PDF reports</small>
    </div>
</div>
...
<div class="d-flex flex-column gap-4">
    ...
    <div class="report-card shadow-sm">
        ...
        <div id="panel-students" class="filter-panel">
            <form method="GET" action="{{ route('partner.reports.students') }}">
                <div class="row g-3">
                    <div class="col-md-3 col-6">
                        ...
                        <select name="session_id" class="form-select form-select-sm">
                            ...
                            <option value="{{ $sess->id }}" {{ $sess->is_active ? 'selected' : '' }}>
    ...
    <div class="report-card shadow-sm">
        <div class="report-card-header" onclick="togglePanel('panel-admissions', this)">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <small class="text-muted">Students admitted by you — filter by session, course, status, gender & date</small>
                </div>
            </div>
            <i class="bi bi-chevron-down collapse-arrow fs-5 text-muted"></i>
        </div>
        ...
        <div id="panel-admissions" class="filter-panel" style="display:none;">
            <form method="GET" action="{{ route('partner.reports.admissions') }}">
                <div class="row g-3">
                    <div class="col-md-3 col-6">
                        <div class="filter-label">Academic Session</div>
                        <select name="session_id" class="form-select form-select-sm">
    ...
    <div class="report-card shadow-sm">
        <div class="report-card-header" onclick="togglePanel('panel-fee', this)">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <small class="text-muted">Fees collected by you — filter by session, course, payment mode & date range</small>
                </div>
            </div>
            <i class="bi bi-chevron-down collapse-arrow fs-5 text-muted"></i>
        </div>
        ...
        <div id="panel-fee" class="filter-panel" style="display:none;">
            <form method="GET" action="{{ route('partner.reports.fee-collection') }}">
                <div class="row g-3">
                    <div class="col-md-3 col-6">
                        <div class="filter-label">Academic Session</div>
                        <select name="session_id" class="form-select form-select-sm">
...
function filterCourses(select, typeId) {
    ...
    allCourses.forEach(function(c) {
        if (!typeId || c.type_id === typeId) {
            ...
            opt.value = c.id; opt.dataset.type = c.type_id; opt.textContent = c.name;
...
function filterStreams(select, courseId) {
    ...
    allStreams.forEach(function(s) {
        if (!courseId || s.course_id === courseId) {
            const opt = document.createElement('option');
            opt.value = s.id; opt.dataset.course = s.course_id; opt.textContent = s.name;
