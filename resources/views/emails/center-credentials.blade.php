...
<head>
    ...
    <title>Your Center Portal Login Credentials</title>
</head>
    ...
    <tr>
        <td align="center">
            <table width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
                ...
                {{-- ── Institute Header ── --}}
                <tr>
                    <td style="background:linear-gradient(135deg,#1e3a5f 0%,#4F46E5 100%);padding:32px 40px;text-align:center;">
                        @if($center->institute?->image)
                            <img src="{{ asset('storage/' . $center->institute->image) }}"
                                 alt="{{ $center->institute->name }}"
                        ...
                        @else
                            <div style="width:64px;height:64px;background:rgba(255,255,255,0.2);border-radius:12px;margin:0 auto 14px;text-align:center;line-height:64px;">
                                <span style="font-size:24px;font-weight:700;color:#fff;">{{ strtoupper(substr($center->institute->name ?? 'I', 0, 2)) }}</span>
                            </div>
                        @endif
                        <h1 style="margin:0;font-size:20px;font-weight:700;color:#ffffff;letter-spacing:0.3px;">{{ $center->institute?->name ?? config('app.name') }}</h1>
                        <p style="margin:6px 0 0;font-size:13px;color:rgba(255,255,255,0.75);">Center Portal — Login Credentials</p>
                    </td>
                </tr>

                {{-- ── Welcome Banner ── --}}
                <tr>
                    <td style="background:#eef2ff;border-bottom:1px solid #c7d2fe;padding:18px 40px;text-align:center;">
                        <p style="margin:0;font-size:16px;font-weight:700;color:#3730a3;">Welcome to the Center Portal!</p>
                        <p style="margin:6px 0 0;font-size:13px;color:#4338ca;">Your center account is ready. Use the details below to get started.</p>
                    </td>
                </tr>
                ...
                <tr>
                    <td style="padding:32px 40px;">
                        ...
                        <p style="margin:0 0 20px;font-size:15px;color:#374151;">
                            Hello <strong>{{ $center->name }}</strong>,
                        </p>
                        <p style="margin:0 0 24px;font-size:15px;color:#374151;line-height:1.7;">
                            Your center account for <strong>{{ $center->institute?->name }}</strong> has been created. Below are your login credentials to access the center portal.
                        </p>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;margin-bottom:24px;overflow:hidden;">
                            <tr>
                                <td style="padding:16px 24px;border-bottom:1px solid #e2e8f0;">
                                    <p style="margin:0;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#64748b;font-weight:600;">Center Code</p>
                                    <p style="margin:4px 0 0;font-size:18px;font-weight:700;color:#4F46E5;letter-spacing:1px;">{{ $center->code ?? '—' }}</p>
                                </td>
                            </tr>
                            <tr>
                                <td style="padding:16px 24px;border-bottom:1px solid #e2e8f0;">
                                    <p style="margin:0;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#64748b;font-weight:600;">Portal URL</p>
                                    <p style="margin:4px 0 0;font-size:14px;color:#4F46E5;font-weight:500;">
                                        <a href="{{ route('center.login') }}" style="color:#4F46E5;text-decoration:none;">{{ route('center.login') }}</a>
                                    </p>
                                </td>
                            </tr>
                            <tr>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
                            <tr>
                                <td align="center">
                                    <a href="{{ route('center.login') }}"
                                       style="display:inline-block;background:linear-gradient(135deg,#4F46E5,#3730a3);color:#ffffff;text-decoration:none;padding:13px 36px;border-radius:8px;font-size:15px;font-weight:700;">
                ...
                <tr>
                    <td style="background:#f8fafc;border-top:1px solid #e2e8f0;padding:20px 40px;text-align:center;">
                        <p style="margin:0 0 4px;font-size:12px;color:#94a3b8;">Powered by <strong style="color:#4F46E5;">College ERP</strong></p>
