...
<head>
    ...
    <title>OTP Verification — Student Portal</title>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;}
        ...
        .bg-glow1{position:fixed;z-index:2;pointer-events:none;width:500px;height:500px;border-radius:50%;background:radial-gradient(circle,rgba(109,40,217,.45) 0%,transparent 70%);top:-150px;left:-100px;animation:floatOrb 8s ease-in-out infinite;}
        .bg-glow2{position:fixed;z-index:2;pointer-events:none;width:400px;height:400px;border-radius:50%;background:radial-gradient(circle,rgba(124,58,237,.4) 0%,transparent 70%);bottom:-120px;right:-80px;animation:floatOrb 10s ease-in-out infinite reverse;}
        ...
        .card-head{background:linear-gradient(135deg,#1e1b4b 0%,#3730a3 50%,#6d28d9 100%);padding:28px 28px 22px;position:relative;overflow:hidden;}
        ...
        .otp-input:focus{border-color:#6d28d9;box-shadow:0 0 0 3px rgba(109,40,217,.12);background:#fff;outline:none;}
        .otp-input.is-invalid{border-color:#ef4444;}
        .btn-submit{height:46px;border-radius:10px;font-size:14px;font-weight:600;background:linear-gradient(135deg,#3730a3,#6d28d9);border:none;color:#fff;letter-spacing:.02em;transition:all .18s ease;width:100%;}
        .btn-submit:hover{background:linear-gradient(135deg,#1e1b4b,#5b21b6);box-shadow:0 6px 20px rgba(109,40,217,.35);transform:translateY(-1px);}
        ...
        .resend-btn{font-size:12px;color:#6d28d9;background:none;border:none;padding:0;cursor:pointer;font-weight:600;}
        .resend-btn:hover{color:#3730a3;}
        ...
        .alert-success-box{background:#f5f3ff;color:#5b21b6;border-left:3px solid #7c3aed;}
    </style>
</head>
<body>
...
<div class="login-wrap">
    <div class="login-card">
        ...
        <div class="card-head">
            ...
            <p style="margin-bottom:6px;">OTP sent to</p>
            @if($student->email)
            <div class="email-chip"><i class="bi bi-envelope-fill"></i>{{ substr($student->email, 0, 3) }}***{{ '@' . explode('@', $student->email)[1] }}</div>
            @endif
            @if($student->mobile)
            <div style="margin-top:6px;font-size:11px;color:rgba(255,255,255,.7);position:relative;z-index:1;">
                <i class="bi bi-phone me-1"></i>Also sent to mobile ••••{{ substr($student->mobile, -4) }}
            </div>
            @endif
        </div>
        ...
        <div class="card-foot">
            ...
            <div class="d-flex align-items-center gap-3">
                <form method="POST" action="{{ route('student.otp.resend') }}" class="mb-0">
                    @csrf
