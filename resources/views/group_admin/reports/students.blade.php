            ...
            <div class="row g-2 align-items-end">
                <div class="col-12 col-md-3">
                    ...
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" name="search" class="form-control border-start-0" placeholder="Name, Father, Mother, Mobile, ID..." value="{{ request('search') }}">
                    </div>
                </div>
                ...
                <div class="col-auto" style="min-width:120px;">
                    ...
                    <select name="course_type_id" id="filterCourseType" class="form-select form-select-sm" onchange="stdFilterCourses(this.value); stdFilterStreams('');">
                ...
                <div class="col-auto" style="min-width:150px;">
                    ...
                    <select name="course_id" id="filterCourse" class="form-select form-select-sm" onchange="stdFilterStreams(this.value);">
                        <option value="">All Courses</option>
                        @foreach($courses as $course)
                            <option value="{{ $course->id }}" data-type="{{ $course->course_type_id }}" {{ request('course_id') == $course->id ? 'selected' : '' }}>{{ $course->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-auto" style="min-width:140px;">
                    ...
                    <select name="course_stream_id" id="filterStream" class="form-select form-select-sm" onchange="stdAutoSubmit()">
                        ...
                        @foreach($streams as $stream)
                            <option value="{{ $stream->id }}" data-course="{{ $stream->course_id }}" {{ request('course_stream_id') == $stream->id ? 'selected' : '' }}>{{ $stream->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-auto" style="min-width:95px;">
                    ...
                    <select name="current_semester" class="form-select form-select-sm" onchange="stdAutoSubmit()">
                        ...
                        @for($s = 1; $s <= $maxSemester; $s++)
                            <option value="{{ $s }}" {{ request('current_semester') == $s ? 'selected' : '' }}>Sem {{ $s }}</option>
                        @endfor
                    </select>
                </div>
                <div class="col-auto" style="min-width:120px;">
                ...
                <div class="col-auto">
                    ...
                    <input type="date" name="from_date" class="form-control form-control-sm" value="{{ request('from_date') }}" style="width:128px;">
                </div>
                <div class="col-auto">
                    <label class="form-label form-label-sm mb-1 text-muted" style="font-size:11px;">To Date</label>
                    <input type="date" name="to_date" class="form-control form-control-sm" value="{{ request('to_date') }}" style="width:128px;">
                </div>
        ...
        <table class="table table-hover table-sm align-middle mb-0" style="font-size:12px;">
            <thead style="background:#1e3a5f; color:#fff;">
                <tr>
                    ...
                    <th style="min-width:90px; white-space:nowrap;">Source</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary-subtle" style="font-size:10px;"><i class="bi bi-calendar3 me-1"></i>{{ $student->session?->name ?? '—' }}</span></td>
                    <td><span class="badge bg-primary bg-opacity-10 text-primary border border-primary-subtle" style="font-size:10px;">{{ $student->student_uid }}</span></td>
                    <td>
                        <div class="fw-semibold" style="font-size:12px;">{{ $student->name }}</div>
                    ...
                    <td>
                        <div class="fw-semibold" style="font-size:12px;">{{ $student->stream?->course?->name ?? '—' }}</div>
                    ...
                    <td class="fw-semibold" style="white-space:nowrap;">
                        ...
                        @if($student->current_semester)
                            <span class="badge bg-primary bg-opacity-10 text-primary border ms-1" style="font-size:9px;">S{{ $student->current_semester }}</span>
                        @endif
                    </td>
                    <td><span class="badge {{ $sourceBadge }} border" style="font-size:10px;">{{ $sourceShort }}</span></td>
                    <td class="fw-semibold text-muted" style="white-space:nowrap;">{{ $student->admission_date?->format('d M Y') ?? '—' }}</td>
                    <td><span class="badge border {{ $statusColor }}" style="font-size:10px;">{{ ucfirst($student->status ?? 'pending') }}</span></td>
                </tr>
                @empty
                <tr><td colspan="11" class="text-center text-muted py-5"><i class="bi bi-people fs-2 d-block mb-2"></i>No students found matching your filters.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div class="px-3 pb-3 pt-2">
        @include('institute.components.pagination', ['paginator' => $students, 'perPage' => 50])
    </div>
</div>
...
(function () {
    ...
    window.stdAutoSubmit = function () { submitFilter(); };
